...
class MenuTabBar extends StatefulWidget {
  ...
  MenuTabBar({Key key, this.child, this.background = Colors.blue, this.iconButtons, this.colorMenuIconActivated = Colors.blue, this.colorMenuIconDefault = Colors.white, this.backgroundMenuIconActivated = Colors.white, this.backgroundMenuIconDefault = Colors.blue, this.changePage})
...
class _MenuTabBar extends State<MenuTabBar> with TickerProviderStateMixin {
  ...
  List<Widget> _buildMenuIcons(){
    ...
    for(var i = 0; i < widget.iconButtons.length; i++){
      if(i == widget.iconButtons.length/2) {
        icons.add(new Container(width: MediaQuery.of(context).size.width/(widget.iconButtons.length+1), height: 0)
        );
      }
  ...
  void _updateButtonPosition(double dy){
    ...
    _animationUp = new Tween<double>(begin: position, end: MediaQuery.of(context).size.height*0.2).animate(new CurvedAnimation(parent: _animationControllerUp, curve: Curves.ease));
  ...
  void _movementCancel(double dy){
    if((MediaQuery.of(context).size.height - dy) < MediaQuery.of(context).size.height*0.2){
      _moveButtonDown();
    }
    else {
      _moveButtonUp();
  ...
  void _finishedMovement(double dy){
    if((MediaQuery.of(context).size.height - dy).round() == (MediaQuery.of(context).size.height*0.2).round()) _isActivated.sink.add(1);
  }
        ...
        new StreamBuilder(stream: _isActivated.stream, builder: (context, AsyncSnapshot<int> snapshot){
          ...
          new StreamBuilder(initialData: 0.0, stream: _opacity.stream, builder: (context, AsyncSnapshot<double> snapshot) {
            ...
            new StreamBuilder(initialData: 0.0, stream: _positionButton.stream, builder: (context, AsyncSnapshot<double> snapshot) {
              var positon = snapshot.data >= MediaQuery.of(context).size.height*0.2 ? (MediaQuery.of(context).size.height*0.2) - (snapshot.data - (MediaQuery.of(context).size.height*0.2)) : snapshot.data;
                ...
                new StreamBuilder(
                  ...
                  return new FloatingActionButton(
                    ...
                    if(_isActivated.stream.value == 1) {
                      ...
                      child: new Icon(FontAwesomeIcons.qrcode, color: snapshot.data == -1 ? widget.colorMenuIconDefault : widget.colorMenuIconActivated),
